...
model = keras.Sequential([
    ...
    layers.Conv2D(64, kernel_size=(3, 3), activation='relu', kernel_regularizer=keras.regularizers.l2(0.001)),
    layers.MaxPooling2D(pool_size=(2, 2)),
    layers.Flatten(),
    layers.Dense(128, activation='relu', kernel_regularizer=keras.regularizers.l2(0.001)),
    layers.Dense(10, activation='softmax', kernel_regularizer=keras.regularizers.l2(0.001))
])
...
log_file = open('training_log_L2_2.txt', 'w')
...
# Train the model while changing the epochs to 30 and batch_size to 64
start_time = time.time()
model.fit(train_images, train_labels, epochs=30, batch_size=64, callbacks=[LogCallback()])
end_time = time.time()
...
model.save('fashion_mnist_model_L2_2.h5')